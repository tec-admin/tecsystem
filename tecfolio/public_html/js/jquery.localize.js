...
(function($) {
  ...
  $.localize = function(pkg, options) {
    ...
    var jikan = new Date();
    var seireki = "" + (jikan.getMonth() + 1) + jikan.getDate() + jikan.getHours() + jikan.getMinutes();
    fileExtension = options.fileExtension || "json" + "?" + seireki;
    ...
    defaultCallback = function(data) {
      ...
      return wrappedSet.each(function() {
        ...
        var num = 0;
        var arr = [];
        while(elem.attr("data-arg" + ++num) != undefined) {
            arr.push(elem.attr("data-arg" + num));
        }
        
        if (value != null && value != "") {
        	// 特殊文字をHTMLエンティティに変換する
        	// ただし、文字をポップアップで表示させる場合は変換しない
        	if(!elem.attr('title'))
        		value = value.htmlspecialchars();
        	
        	if(arr.length > 0) {
        		value = value.sprintf(arr);
        	}
        	return localizeElement(elem, key, value);
        }
        else {
        	// 翻訳ファイルと一致しない場合、data-localizeの値を原文として扱う
        	if(arr.length > 0) {
        		key = key.sprintf(arr);
        	}
        	return localizeElement(elem, key, key);
        }
      });
    };
