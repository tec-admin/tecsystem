...
<script>
	...
	function disp()
	{
		var sf = document.getElementById('selected_form').value;
		var df = document.getElementById('startdate_hidden').value;
		var dt = document.getElementById('enddate_hidden').value;
		createStatistics(sf, df, dt);
	}

	function createStatistics(form, startdate, enddate)
	{
		deleteStatistics();
		initGlobals();
		...
		var scripturl = baseurl + "/getbyreserveform/form/" + form + "/startdate/" + startdate + "/enddate/" + enddate;
		...
		*/
		...
		$.ajax({
			async: true,	// 非同期通信
			url: baseurl + "/getbyreserveform/form/" + form + "/startdate/" + startdate + "/enddate/" + enddate,
			type: "POST",
			timeout: 600000,
			datatype: 'json',
			...
			success: function(data, textStatus, jqXHR) {
				...
				for (var dow in staffshift['shiftinput'])
				{
					for (var dayno in staffshift['shiftinput'][dow])
					{
						...
						var count_total = staffshift['shiftinput'][dow][dayno]['count_total'];
						var count_run = staffshift['shiftinput'][dow][dayno]['count_run'];
		
						if(count_total != undefined && count_run != undefined)
						{
							var percent = Math.round(count_run / count_total * 100);
						}
						else
						{
							var percent = 0;
						...
						if(count_run != undefined)
						{
							td.innerHTML += '( ' + count_run + ' / ';
		
							avg_time[dayno-1][0] += count_run;
							avg_week[dow-1][0] += count_run;
							avg_sum[0] += count_run;
						}
						else
						{
						...
						if(count_total != undefined)
						{
							td.innerHTML += count_total + ' )';
		
							avg_time[dayno-1][1] += count_total;
							avg_week[dow-1][1] += count_total;
							avg_sum[1] += count_total;
						}
						else
						{
							td.innerHTML += '0 )';
	...
	{
		$('#displaybutton').removeClass("inactive");
		$('#displaybutton').removeAttr("disabled");
	}
</script>
</head>
						...
						<table>
							<thead>
								<tr>
									...
									<th>
										<select id="selected_form" onchange="buttonactive()">
											<option id="" selected="selected" disabled="disabled">{t}選択してください{/t}</option>
											<option value="0">{t}事前予約件数{/t}</option>
											<option value="1">{t}駆け込み件数{/t}</option>
										</select>
									</th>
									<th class="disp">
										<input type="button" id="displaybutton" class="inactive" disabled="disabled" value="{t}表示{/t}" onclick="disp()" style="position: relative ; top:1px;">
									</th>
								</tr>
							</thead>
						</table>
