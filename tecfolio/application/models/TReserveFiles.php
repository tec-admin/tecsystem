...
class Class_Model_TReserveFiles extends BaseTModels
{
	const TABLE_NAME = 't_reserve_files';
	protected $_name   = Class_Model_TReserveFiles::TABLE_NAME;

	public static function fieldArray($prefix = '')
	{
		if ($prefix == '')
			$prefix = Class_Model_TReserveFiles::TABLE_NAME;
	...
	public function selectFromReserveId($t_reserve_id)
	{
		...
		$select->setIntegrityCheck(false)
				->from(
					array('reservefiles' => $this->_name), "*")

				->join(
					array('work_files' => 't_work_files'),
					'work_files.id = reservefiles.t_work_file_id',
				...
				->join(
					...
					'members.id = reservefiles.creator',
					Class_Model_MMembers::fieldArray()
				)

				
				->where('reservefiles.t_reserve_id = ?', $t_reserve_id)
